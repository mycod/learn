...
with tf.Session() as sess:
    ...
    for cycle in range(20): # 20次训练
        for batch in range(batch_num): # 分批次训练，一批反向传播修改一次参数
            batch_data_x, batch_data_y = mnist.train.next_batch(batch_size)
            sess.run(train, feed_dict={x: batch_data_x, y: batch_data_y,
                                       keep_prob: 0.7})
        # 训练完一次计算准确率, 显存不足，分批求精确度再求平均
        acc_list =[]
        for _ in range(test_num):
            batch_test_x, batch_test_y = mnist.test.next_batch(test_size)
            step_accuracy = sess.run(accuracy, feed_dict={x: batch_test_x,
                                    y: batch_test_y, keep_prob: 1.0})
            acc_list.append(step_accuracy)
        acc = sum(acc_list) / len(acc_list)
        print(acc_list)
        print("cycle: ", cycle, 'Test accuracy:', acc)
        saver.save(sess, 'conv_net/my_net.ckpt')
